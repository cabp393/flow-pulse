...
const createMaster = (): SkuMaster => ({
  ...
  createdAt: new Date().toISOString(),
  rows: [
    { sku: 'A', locationId: 'L-A', sequence: 1 },
    { sku: 'B', locationId: 'L-A', sequence: 2 },
    { sku: 'C', locationId: 'L-C', sequence: 3 },
  ],
  index: {
...
describe('buildRun', () => {
  it('builds pickPlan ordered by sequence and keeps deterministic dedupe by location', async () => {
    ...
    const { run } = await buildRun(createLayout(), createMaster(), lines, 'sample.xlsx');
  ...
  it('selects the minimum sequence option when SKU has multiple mapped locations', async () => {
    ...
    const { run } = await buildRun(createLayout(), master, [{ pallet_id: 'P1', sku: 'Z' }], 'sample.xlsx');
